...
def est_bit_proportion(byteArray):
    ...
    return (numberOfOnes/(lengthOfByteArray * 8))
...
def est_extract():
    ...
    valuesForP = [1, 2, 3]
    valuesForQ = [1, 2, 3]
    foundCorrectValues = False
    
    for x in valuesForP:
        for y in valuesForQ:
            message_bit_array = est_extract_bits_from_image(image_byte_array, header_len, x, y )
            
            bp = est_bit_proportion(est_convert_bits_to_bytes( message_bit_array ))
            
            if (bp < 0.5):
                p = x
                q = y
                foundCorrectValues = True
                
            print("p = " , x , " q = " , y , " bp = " , round(bp, 3))

    if (foundCorrectValues):
        print("The answer is :\np = " , p , " q = " , q)
    else:
        print("Could not find correct values for p and q!")
